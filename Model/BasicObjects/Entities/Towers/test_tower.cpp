...
TestTower::TestTower(const QPointF& coordinates)
  : Tower(coordinates, ":images/test_tower.png"),
    attack_timer_(this), range_(1000), local_attack_area_(CreateAttackArea(range_)) {
  attack_timer_.setSingleShot(true);
  attack_timer_.setInterval(1000);

  // TODO change it when coordinates are changed
...
void TestTower::Tick(Time delta) {
  ...
  if (fired) return;
  QList<QGraphicsItem*> items_in_attack_area = scene()->items(scene_attack_area_);
  ...
  //  QList<QGraphicsItem*> items_in_attack_area = scene()->items(local_attack_area_);
  //  for (QGraphicsItem* item : items_in_attack_area) {
